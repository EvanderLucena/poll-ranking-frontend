export default function Button({ children, className = "", ...props }) {
  ...
  return (
    <button
      className={
        `rounded-xl px-4 h-12 font-medium shadow-sm border flex items-center justify-center ${defaultBg} ${defaultText} ${defaultBorder} hover:bg-opacity-90 active:scale-[0.99] transition-all ` +
        className
      }
      {...props}
    >
